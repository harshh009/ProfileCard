...
// Best MEthod => Destructuring Props 
import "./styles.css";
const skills = [
  {
    skill: "HTML+CSS",
...
function SkillSet() {
  return (
    <ul className="skill-list">
      {skills.map((s) => (
        <Skills skill={s.skill} color={s.color} level={s.level} />
      ))}
    </ul>
  );
}
...
function Skills({ skill, color, level }) {
  ...
  return (
    <>
      <l1 className="skill" style={{ backgroundColor: color }}>
        <h2>{skill}</h2>
        <span>{l}</span>
      </l1>
    </>
...
// USING SKILL ONGECT TO FILL PROPS DATA
